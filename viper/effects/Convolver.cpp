...
void Convolver::PrepareKernelBuffer(
    const uint32_t buffer_size, const uint32_t channel_count, const int32_t reset_flag
) {
    if (reset_flag == 0) {
        if (channel_count - 1 < 2 && buffer_size > 0) {
            ...
            delete[] kernel_buffer_;
            kernel_buffer_ = new float[buffer_size];
...
void Convolver::SetCrossChannel(float cross_channel) {
    if (cross_channel <= 0.0f) {
        is_valid_cross_channel_ = false;
        return;
    }

    if (cross_channel > 1.0f) {
        cross_channel = 1.0f;
    }
...
void Convolver::SetKernel(const float *buf, const uint32_t len) {
    if (len < 16) return;

    kernel_ch1_.Reset();
    kernel_ch2_.Reset();

    const uint32_t ret1 = kernel_ch1_.LoadKernel(buf, len, kConvSegmentSize);
    const uint32_t ret2 = kernel_ch2_.LoadKernel(buf, len, kConvSegmentSize);
    if (ret1 == 0 || ret2 == 0) {
        kernel_ch1_.UnloadKernel();
        kernel_ch2_.UnloadKernel();
    }
...
void Convolver::SetKernelBuffer(const float *buf, uint32_t len) {
    if (buf == nullptr || len == 0) return;
    if (kernel_buffer_ == nullptr || expected_size_ == 0) return;

    if (current_size_ >= expected_size_) return;
    if (current_size_ + len > expected_size_) {
        len = expected_size_ - current_size_;
    }

    memcpy(kernel_buffer_ + current_size_, buf, len * sizeof(float));
    current_size_ += len;
}

void Convolver::SetKernelStereo(
    const float *left_channel, const float *right_channel, const uint32_t frame_count
) {
    ...
    const uint32_t ret1 =
        kernel_ch1_.LoadKernel(left_channel, frame_count, kConvSegmentSize);
    const uint32_t ret2 =
        kernel_ch2_.LoadKernel(right_channel, frame_count, kConvSegmentSize);
    if (ret1 == 0 || ret2 == 0) {
        kernel_ch1_.UnloadKernel();
        kernel_ch2_.UnloadKernel();
    }
